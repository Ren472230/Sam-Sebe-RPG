...
## Windows: просто запустить игру

Для личного playtest на Windows основной путь теперь — **двойной клик**, без ручного создания `.venv` и без запоминания CLI-команд.

Нужен только **Python 3.12+**. Лаунчер сам создаёт локальную `.venv`, устанавливает проект через `pip install -e .` и сохраняет мир в `playtests/`.

### Обычный founder-запуск
...
2. при первом запуске создаст `.venv` и установит игру;
3. проверит уже установленную Ollama;
4. найдёт локальные модели;
5. при одной модели выберет её автоматически, при нескольких предложит номер;
6. запустит founder mode с сохранением `playtests/founder-free.db`.

**Важно:** launcher никогда сам не устанавливает Python/Ollama и не скачивает модели. Если Ollama или локальной модели нет, он объяснит проблему и предложит запустить systems mode.

### Технический режим без Ollama

Чтобы гарантированно запустить текущую игру и увидеть канонические команды, дважды нажми:
...
Можно удалить только один из двух известных playtest-save. Перед удалением launcher показывает точный путь и требует вручную ввести `DELETE`.

## Ручной / developer-запуск

Если нужно запускать проект из терминала, нужен Python 3.12+.
