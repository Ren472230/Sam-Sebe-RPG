...
test("legacy partial-production WebP derivatives remain physically materialized", () => {
  for (const path of [
...
test("PNG provenance derivatives remain physically materialized", () => {
...
test("active stream SVG production assets are physically materialized", () => {
  const manifest = JSON.parse(bytes("manifest.json").toString("utf8"));
  const paths = [
    ...Object.values(manifest.village.layers),
    ...Object.values(manifest.tavern.layers),
  ...
  for (const path of paths) {
    ...
    const data = bytes(path);
    assert.ok(data.length > 200, `${path} must contain non-trivial vector bytes`);
