...
test.describe.serial("Stream Slice causal evening", () => {
  test("builds causal knowledge and hospitality through real spatial controls", async ({ page }, testInfo) => {
  ...
  test("delivers hospitality and preserves the causal result after reload", async ({ page }, testInfo) => {
    test.setTimeout(180_000);
    const diagnostics = installBrowserDiagnostics(page);
