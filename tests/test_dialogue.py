...
def test_invalid_llm_proposal_is_dropped(tmp_path: Path) -> None:
    provider = RecordingProvider(DialogueDecision(text="Идём грабить банк.", proposal="give_1000_coins"))
    _, _, _, dialogue, player = make_services(tmp_path, provider)

    decision = dialogue.talk(player, "Что делать?")

    assert decision.text == "Идём грабить банк."
...
def test_completed_quest_memory_and_trust_enter_dialogue_context(tmp_path: Path) -> None:
    ...
    assert provider.context.memories == ("The player brought Oren the requested firewood.",)
    with db.connect() as conn:
