...
class GameDatabase:
    ...
    def _bootstrap(self, conn: sqlite3.Connection) -> None:
        ...
        conn.execute("INSERT OR IGNORE INTO worlds (id, name, timezone, created_at, last_simulated_at) VALUES (?, ?, ?, ?, NULL)", (DEFAULT_WORLD_ID, "MVP Village", "UTC", created_at))
        conn.executemany("INSERT OR IGNORE INTO locations (id, world_id, name, description, sort_order) VALUES (?, ?, ?, ?, ?)", [(location_id, DEFAULT_WORLD_ID, name, description, sort_order) for location_id, name, description, sort_order in _LOCATIONS])
        conn.executemany("INSERT OR IGNORE INTO location_edges (from_location_id, to_location_id) VALUES (?, ?)", _EDGES)
        for actor_id, name, location_id, role, activity in _NPCS:
            conn.execute("INSERT OR IGNORE INTO actors (id, world_id, actor_type, name, location_id, created_at) VALUES (?, ?, 'npc', ?, ?, ?)", (actor_id, DEFAULT_WORLD_ID, name, location_id, created_at))
            conn.execute("INSERT OR IGNORE INTO npcs (actor_id, role, current_activity) VALUES (?, ?, ?)", (actor_id, role, activity))
        conn.executemany("INSERT OR IGNORE INTO npc_schedule (npc_actor_id, start_minute_local, end_minute_local, location_id, activity, priority) VALUES (?, ?, ?, ?, ?, ?)", _SCHEDULE)
        conn.executemany("INSERT OR IGNORE INTO entities (id, world_id, name, entity_type, location_id, owner_actor_id, portable, state_json, created_at) VALUES (?, ?, ?, ?, ?, NULL, ?, ?, ?)", [(entity_id, DEFAULT_WORLD_ID, name, entity_type, location_id, portable, json.dumps(state, separators=(",", ":"), sort_keys=True), created_at) for entity_id, name, entity_type, location_id, portable, state in _ENTITIES])
