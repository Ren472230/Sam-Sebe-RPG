...
BOOTSTRAP_ENTITIES: tuple[tuple[str, str, str, str | None, list[str], dict[str, Any]], ...] = (
    ...
    ("mira_craftswoman", "npc", "Мира, ремесленница", "workshop_yard", ["npc"], {}),
    ("oren_innkeeper", "npc", "Орен, трактирщик", "village_square", ["npc"], {}),
    ("kaspar_forager", "npc", "Каспар, собиратель", "river_edge", ["npc"], {}),
    ("raven_1", "animal", "Ворон", "village_square", ["animal", "raven"], {"trust": 0, "fear": 0}),
    ("raven_2", "animal", "Молодой ворон", "river_edge", ["animal", "raven"], {"trust": 0, "fear": 0}),
    ("target_barrel", "object", "Старая бочка", "workshop_yard", ["throw_target"], {}),
    ("target_sign", "object", "Вывеска трактира", "village_square", ["throw_target"], {}),
    ("target_post", "object", "Корявая стойка", "river_edge", ["throw_target"], {}),
    ("stone_flat_1", "item", "Плоский камень", "workshop_yard", ["improvised_projectile", "flat_stone"], {}),
    ("stone_round_1", "item", "Круглый камень", "workshop_yard", ["improvised_projectile", "round_stone"], {}),
    ("pinecone_1", "item", "Сосновая шишка", "river_edge", ["improvised_projectile", "pinecone"], {}),
...
class GameDatabase:
    ...
    def bootstrap_if_empty(self) -> None:
        with self.connect() as conn:
            existing = conn.execute("SELECT COUNT(*) FROM player_state").fetchone()[0]
            if existing:
                return
            for entity_id, entity_type, name, location_id, tags, state in BOOTSTRAP_ENTITIES:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (entity_id, entity_type, name, location_id, json.dumps(tags), json.dumps(state)),
                )
            conn.execute(
                "INSERT INTO player_state(player_id, location_id) VALUES (?, ?)",
    ...
    def fetch_entity(self, entity_id: str) -> dict[str, Any] | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT entity_id, entity_type, name, location_id, tags_json, state_json FROM entities WHERE entity_id = ?",
                (entity_id,),
            ).fetchone()
        if not row:
            return None
    ...
    def list_inventory(self, player_id: str) -> list[str]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT item_id FROM inventory WHERE player_id = ? ORDER BY item_id",
                (player_id,),
            ).fetchall()
    ...
    def fetch_behavior_profile(self, player_id: str, behavior_key: str) -> dict[str, Any] | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT data_json FROM behavior_profiles WHERE player_id = ? AND behavior_key = ?",
    ...
    def fetch_ability(self, player_id: str, ability_id: str) -> dict[str, Any] | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT ability_id, mechanic_json, unlocked_at FROM abilities WHERE player_id = ? AND ability_id = ?",
                (player_id, ability_id),
            ).fetchone()
        if not row:
            return None
