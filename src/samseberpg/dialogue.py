...
class DialogueService:
    ...
    def talk(self, player_id: str, user_text: str) -> DialogueDecision:
        ...
        try:
            ...
            return _fallback(context)
        proposal = decision.proposal if decision.proposal in _ALLOWED_PROPOSALS else None
    ...
    def build_context(self, player_id: str, user_text: str = "") -> DialogueContext:
        ...
        try:
            npc = conn.execute(
                ...
                "FROM npcs JOIN actors ON actors.id = npcs.actor_id WHERE npcs.actor_id = 'npc_oren'"
            ).fetchone()
            if npc is None:
