...
class GameService:
        ...
        with self.db.connect() as conn:
            ...
            try:
                ...
                actor_id = (
                    "player_"
                    + uuid5(
                        NAMESPACE_URL, f"sam-sebe-rpg:{discord_user_id}"
                    ).hex[:16]
                )
                conn.execute(
                    """
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, 10)
                    """,
                    (actor_id, discord_user_id, now_text),
                )
                conn.commit()
                return actor_id
            except Exception:
        ...
        with self.db.connect() as conn:
            ...
            try:
                ...
                actor = conn.execute(
                    ...
                    FROM actors
                    WHERE id = ? AND actor_type = 'player'
                    """,
                    (action.actor_id,),
                ).fetchone()
        ...
        with self.db.connect() as conn:
            ...
            try:
                ...
                exit_rows = conn.execute(
                    ...
                    FROM location_edges
                    WHERE world_id = ? AND from_location_id = ?
                    ORDER BY to_location_id
                    """,
                    (WORLD_ID, player["location_id"]),
                ).fetchall()
                conn.commit()
            except Exception:
                conn.rollback()
    ...
    def _move(
        ...
        return ActionResult(
            True,
            "OK",
            f"Вы переходите в {destination}.",
            data={"location_id": destination},
        )
    ...
    def _take(
        ...
        entity = conn.execute(
            """
            SELECT id, name, location_id, owner_actor_id, portable, state_json
            FROM entities
            WHERE id = ?
            """,
            (action.target_id,),
        ...
        return ActionResult(
            True,
            "OK",
            f"Вы берёте {entity['name']}.",
            data={"entity_id": action.target_id},
        )
    ...
    def _throw(
        ...
        if item_state.get("throwable") is not True:
            return ActionResult(
                False,
                "ITEM_NOT_THROWABLE",
                "Этот предмет нельзя осмысленно бросить в цель.",
            )
        ...
        if isinstance(condition, bool) or not isinstance(condition, int):
            return ActionResult(
                ...
                "Эта цель пока не поддерживает повреждения.",
            )
        ...
        witnesses = [
            ...
            for row in conn.execute(
                ...
                SELECT id
                FROM actors
                WHERE actor_type = 'npc' AND location_id = ?
                ORDER BY id
                """,
                (location_id,),
            ).fetchall()
    ...
    def _give(
        ...
        if action.target_id is None:
            return ActionResult(
                ...
                "Не указано, кому передать предмет.",
            )
        ...
        if target["id"] == action.actor_id:
            return ActionResult(
                False,
                "INVALID_TARGET",
                "Нельзя передать предмет самому себе.",
            )
        if target["location_id"] != location_id:
            return ActionResult(
                False,
                "TARGET_NOT_PRESENT",
                "Этого персонажа здесь нет.",
            )
    ...
    def _buy(
        ...
        item = conn.execute(
            """
            SELECT id, name, location_id, owner_actor_id, state_json
            FROM entities
            WHERE id = ?
            """,
            (action.item_id,),
        ...
        if action.target_id != seller_id:
            return ActionResult(
                False,
                "WRONG_SELLER",
                "Этот товар продаёт другой персонаж.",
            )
    ...
    def _use(
        ...
        target_state = json.loads(target["state_json"])
        if (
            item_state.get("fillable") is not True
            or target_state.get("water_source") is not True
        ):
            return ActionResult(
                False,
                "UNSUPPORTED_USE",
                "Так использовать этот предмет пока нельзя.",
            )
        ...
        conn.execute(
            "UPDATE entities SET state_json = ? WHERE id = ?",
            (
                json.dumps(item_state, ensure_ascii=False, sort_keys=True),
                action.item_id,
            ),
        )
        evidence = {
    ...
    def _apply_relation_delta(
        ...
        *,
        ...
        before = {
            field: int(row[field]) if row is not None else 0
            for field in fields
        }
        after = before.copy()
        actual: dict[str, int] = {}
